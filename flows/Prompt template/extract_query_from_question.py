...
# Function to check if the query or output is disaster-related
...
# Function to generate a response for the first question
def initial_prompt(query, llm_with_tools):
    # Infer user intent from query
    refined_query = infer_user_intent(query)
    ...
    # Check if the response is disaster-related
    if not is_disaster_related(response.content):
        return "Sorry, I can only answer questions related to disasters. Please ask a more specific question."

    return response.content

# Function to handle follow-up queries using the previous context
def chain_prompt(previous_output, query, llm_with_tools, chat_history):
    ...
    if not is_disaster_related(refined_query):
    ...
    # Check if the response is disaster-related
    if not is_disaster_related(chain_response.content):
        return "Sorry, I can only answer questions related to disasters. Please ask a more specific question."
